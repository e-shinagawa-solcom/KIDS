...
function fncGetStockSubject(objout, stockclasscode) {
	$.ajax({
		url: "/cmn/getdropdowndata.php",
		type: 'post',
		data: {
			'lngProcessID': "cnStockSubjectCode",
			'strFormValue': stockclasscode
		}
	})
		.done(function (response) {
			var data = JSON.parse(response);
			$('select[name="lngstocksubjectcode"] option').remove();
			$('select[name="lngstocksubjectcode"]').append("<option value=''></option>");
			$('select[name="lngstocksubjectcode"]').append(data.pulldown);
		})
