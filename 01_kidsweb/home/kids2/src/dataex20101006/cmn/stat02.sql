...
FROM
	...
	(
		...
		/* 売上マスタ・計上日に対する条件設定 */
		AND to_date( ms.dtmAppropriationDate,'YYYY/MM/DD' ) >= to_date( '_%dtmAppropriationDateFrom%_', 'YYYY/MM/DD' ) /* FROM */
		AND to_date( ms.dtmAppropriationDate,'YYYY/MM/DD' ) <= to_date( '_%dtmAppropriationDateTo%_', 'YYYY/MM/DD' ) /* TO */
	) mst1
