...
try
{
	$moldReportId = $_REQUEST[FormMoldReport::MoldReportId];
	$revision = $_REQUEST[FormMoldReport::Revision];
	$version = $_REQUEST[FormMoldReport::Version];

	// 金型帳票ID, リビジョン, バージョン
	$utilMold->selectMoldReport($moldReportId, $revision, $version);
}
catch (SQLException $e)
{
	$errMstList[FormMoldReport::MoldReportId] =
		"[金型帳票ID]->マスタ上に存在しない値か対象データが変更されています。:".
		$_REQUEST[FormMoldReport::MoldReportId];
}
