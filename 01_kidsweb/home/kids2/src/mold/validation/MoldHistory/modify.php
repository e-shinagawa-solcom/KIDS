...
{
	$errMstList[FormMoldHistory::Status] =
		"[ステータス]->業務コードマスタ上に存在しない値です。:".
		$_REQUEST[FormMoldHistory::Status];
}
// 実施日 -> 日付フォーマットチェック
...
// 金型NO
$moldNo = $_REQUEST[FormMoldHistory::MoldNo];

// 金型NO/金型説明の件数が0件の場合
if (!$moldNo)
{
	$errMstList["MoldNo"] =
			"[金型NO]->金型NOの指定がありません。。:".count($molds);
}
// 金型NO/金型説明の個数が正常の場合
else
{
