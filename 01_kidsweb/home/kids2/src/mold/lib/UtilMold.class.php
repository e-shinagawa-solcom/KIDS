...
class UtilMold extends WithQuery
{
     ...
     * @return 取り込み件数
     */
    public function updateMoldToValid()
    {
        $result = false;
    ...
    public function selectMaxMoldHistoryNo($moldNo, $objDB)
    {
        unset($aryQuery);
        $aryQuery[] = "select max(historyno) historyno from t_moldhistory ";
        $aryQuery[] = "WHERE";
        $aryQuery[] = "  moldno = '" . $moldNo . "'";
        $aryQuery[] = "  and deleteflag = false";
		$strQuery = implode("\n", $aryQuery);		
        list($lngResultID, $lngResultNum) = fncQuery($strQuery, $objDB);
        if (!$lngResultID) {
			throw new SQLException(
                "金型履歴データの最大履歴番号の取得が失敗しました。",
                $strQuery,
                null
            );
        }
