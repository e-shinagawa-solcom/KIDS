...
function fncGetEstimate( $lngEstimateNo, $objDB )
{
	...
	// 最終承認者名の取得
	//////////////////////////////////////////////////////////
	$aryQuery[] = "SELECT distinct on (e.lngEstimateNo) e.lngEstimateNo";
	// 最終承認者名
	$aryQuery[] = ", inchag_u2.strUserFullName as strInChargeUserName";

	$aryQuery[] = " FROM m_Estimate e";
	// ワークフロー-発注
	$aryQuery[] = " LEFT JOIN m_Workflow w ON ( e.lngEstimateNo = to_number(w.strWorkflowKeyCode, '9999999') AND w.lngFunctionCode = " . DEF_FUNCTION_E0 . ")";
	// ワークフロー-ワークフロー順番
	$aryQuery[] = " LEFT JOIN m_WorkflowOrder wo ON ( w.lngWorkflowOrderCode = wo.lngWorkflowOrderCode )";
	// ワークフロー順番-ユーザー
	$aryQuery[] = " LEFT JOIN m_User inchag_u2 ON ( wo.lngInChargeCode = inchag_u2.lngUserCode )";

	$aryQuery[] = " WHERE e.lngEstimateNo = " . $lngEstimateNo;
	//$aryQuery[] = " AND wo.lngWorkflowStatusCode = " . DEF_FINAL_APPROVER;
	// D:「承認」状態にある案件
	$aryQuery[] = " AND (";

	// D:「承認」状態にある案件
	$aryQuery[] = "      " . DEF_STATUS_APPROVE . " = ";
	$aryQuery[] = "      (";
	$aryQuery[] = "        SELECT distinct on (tw.lngWorkflowStatusCode) lngWorkflowStatusCode ";
	$aryQuery[] = "        FROM m_Workflow mw2, t_Workflow tw";
	$aryQuery[] = "        WHERE to_number ( mw2.strWorkflowKeyCode, '9999999') = e.lngEstimateNo";
	$aryQuery[] = "         AND mw2.lngFunctionCode = " . DEF_FUNCTION_E0;
	$aryQuery[] = "         AND mw2.lngWorkflowCode = ";
	$aryQuery[] = "         (";
	$aryQuery[] = "           SELECT MAX ( mw3.lngWorkflowCode ) FROM m_Workflow mw3 ";
	$aryQuery[] = "           WHERE mw2.strWorkflowKeyCode = mw3.strWorkflowKeyCode ";
 	$aryQuery[] = "         )";
	 ...
	 *	受注マスタ（m_Receive）を売上マスタ（m_Sales）に
	 *	受注明細テーブル（t_ReceiveDetail）を売上明細テーブル（t_SalesDetail）に変更
	 */
...
//	$aryQuery[] = "SELECT distinct r.strReceiveCode, rd.lngReceiveDetailNo, rd.lngProductQuantity AS lngProductQuantity, rd.lngProductUnitCode AS lngProductUnitCode, ";
...
//	$aryQuery[] = "LEFT JOIN t_ReceiveDetail rd ON ( r.lngReceiveNo = rd.lngReceiveNo ) ";
...
//	$aryQuery[] = " AND r.lngRevisionNo = (SELECT MAX(r2.lngRevisionNo) FROM m_Receive r2 WHERE r.strReceiveCode = r2.strReceiveCode )";
...
//	$aryQuery[] = " AND 0 <= ( SELECT MIN( r3.lngRevisionNo ) FROM m_Receive r3 WHERE r3.bytInvalidFlag = false AND r3.strReceiveCode = r.strReceiveCode )";
...
/**
	...
	$aryQuery[] = " LEFT JOIN m_Order o ON ( od.lngOrderNo = o.lngOrderNo ) ";
	...
	$aryQuery[] = "SELECT MAX( o2.strReviseCode ) FROM m_Order o2 WHERE o2.strOrderCode = o.strOrderCode )";
	$aryQuery[] = " AND 0 <= ( ";
...
//	$aryQuery[] = " AND o.strReviseCode = ( ";
//	$aryQuery[] = "SELECT MAX( o2.strReviseCode ) FROM m_Order o2 WHERE o2.strOrderCode = o.strOrderCode )";
//	$aryQuery[] = "SELECT MAX( o2.strReviseCode ) FROM m_Stock o2 WHERE o2.strStockCode = o.strStockCode )";
