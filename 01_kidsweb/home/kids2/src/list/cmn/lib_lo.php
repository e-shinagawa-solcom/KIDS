...
function fncGetListOutputQuery( $lngClassCode, $lngKeyCode, $objDB )
{
	...
	elseif ( $lngClassCode == DEF_REPORT_ORDER )
	{
		...
		// 入力者
		//$aryQuery[] = ", incharge_u.strUserDisplayCode as strInChargeUserDisplayCode";
		//$aryQuery[] = ", incharge_u.strUserDisplayName as strInChargeUserDisplayName";
		// 仕入先
...
//39期に対応 担当者→Group名称使用
		...
		$aryQuery[] = " FROM m_Order o";
		// ユーザー-発注
		//$aryQuery[] = " LEFT JOIN m_User incharge_u ON o.lngUserCode = incharge_u.lngUserCode";
		// 発注-会社
		...
		// ワークフロー-発注
		//$aryQuery[] = " LEFT JOIN m_Workflow w ON w.strWorkflowKeyCode = o.lngOrderNo";
		$aryQuery[] = " LEFT JOIN m_Workflow w ON w.strWorkflowKeyCode = trim(to_char(o.lngOrderNo, '9999999'))";
		// ワークフロー-ワークフロー順番
		$aryQuery[] = " LEFT JOIN m_WorkflowOrder wo ON wo.lngWorkflowOrderCode = w.lngWorkflowOrderCode";
		// ワークフロー順番-ユーザー
