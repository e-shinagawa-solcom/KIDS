...
//		$worksheet->writeString( 1, 10, $aryData["dtmAppropriationDateFrom"] ,$objFormat);
...
return 3;
	...
	*
	*	$aryKeyCol の第一番目に指定されるカラム数値は、重要な意味を持つ。（指定する順序が関係する）
	* 	ここに指定された第一番目は、その値をキーとして異なっていた場合、次に続くカラム数値を全て
	*	小計行の対象として表示させるよう、処理するロジックになっている。
	//****************************************************************************
	*/
	function fncSpreadSheetCellData_SalesKeyCol($aryData)
	{
		...
		{
			$aryKeyCol = array(5, 3);	// 3顧客コード、5部門コード
		}
		else
		{
			$aryKeyCol = array(5, 10);	// 5部門コード、3製品コード
		}
