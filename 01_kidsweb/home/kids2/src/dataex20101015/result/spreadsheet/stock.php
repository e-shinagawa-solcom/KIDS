...
*/
	...
	*/
	function fncSpreadSheetCellData_StockKeyCol($aryData)
	{
		...
		{
			$aryKeyCol = array(3, 9);	// 仕入先コード、仕入科目コード
		}
		else
		{
			$aryKeyCol = array(5, 9, 13);	// 部門コード、仕入科目コード、製品コード
		}
	...
	//	小計行を追加（描画）する
	*
	*	@param	integer	$lngRet			小計行数（増やす）
	*	@param	integer	$i				行カウンター
	*	@param	integer	$lngHeadLineCnt	ヘッダ行数
	*	@param	integer	$lngSubtotal	小計行数（既存インクリメント分）
	*	@param	object	$workbook		ワークブック
	*	@param	object	$worksheet		ワークシート
	*	@param	array	$aryFormat		フォーマット配列
	*	@param	array	$aryDataset		データセット配列
	*	@param	integer	$arySubTotalBuff	計算バッファ
	*
	*	@return	Integer	追加した小計行数
	*
	//
	//****************************************************************************
	*/
