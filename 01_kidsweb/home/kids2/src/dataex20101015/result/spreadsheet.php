...
//exit;
		...
		*/
		...
		// ヘッダー部、最初のx行を固定する-> 機能してない
//		$worksheet->freezePanes(array($lngHeadLineCnt, 0));
//		$worksheet->thawPanes(array($lngHeadLineCnt, 0));
...
//					$arySubTitle = fncSpreadSheetCellData_StockSubTitle();
					...
					// 小計行数を判断する
					$lngRet = fncSpreadSheetCellData_SubTotalCount($aryKeyCol, $aryDataset, $i, $aryResult, $lngResultRows, $lngResultID, $objDB);
					...
					if( $lngRet > 0 )
					{
						...
						// 小計行の追加
						$lngAddLine = fncSpreadSheetCellData_SubTotalAdd($arySubTitle, $lngRet, $i, $lngHeadLineCnt, $lngSubtotal, $workbook, $worksheet, $aryFormat, $aryData, $aryDataset, $arySubTotalBuff, 15, 19 );
					}
					break;
				...
				case 4:
					...
					if( $lngRet > 0 )
					{
						...
						$arySubTitle = fncSpreadSheetCellData_StockSubTitle();
						// 小計行の追加
						$lngAddLine = fncSpreadSheetCellData_SubTotalAdd($arySubTitle, $lngRet, $i, $lngHeadLineCnt, $lngSubtotal, $workbook, $worksheet, $aryFormat, $aryData, $aryDataset, $arySubTotalBuff, 20, 24 );
					}
					break;
				...
				case 2:
					...
					if( $lngRet > 0 )
					{
						...
						// 小計行の追加
						$lngAddLine = fncSpreadSheetCellData_SubTotalAdd($arySubTitle, $lngRet, $i, $lngHeadLineCnt, $lngSubtotal, $workbook, $worksheet, $aryFormat, $aryData, $aryDataset, $arySubTotalBuff, 23, 25 );
					}
					break;
			}
			//-----------------------------------------------------------------------------------------------------
	...
	*/
	function fncSpreadSheetCellData_SubTotalCount($aryKeyCol, $aryDataset, $i, $aryResult, $lngResultRows, $lngResultID, $objDB)
	{
		...
		// 1つ先の情報が同一であるかを確認
		// 又は最後の行の場合、小計行を表示させる
		$lngRet = fncSpreadSheetCellData_Diff($aryDataset, $i, $aryKeyCol, $aryResult, $aryAfterResult);
	...
	*
	...
	//****************************************************************************
	*/
	function fncSpreadSheetCellData_SubTotalAdd($arySubTitle, $lngRet, $i, $lngHeadLineCnt, $lngSubtotal, &$workbook, &$worksheet, $aryFormat, $aryData, $aryDataset, &$arySubTotalBuff, $lngStartCol, $lngEndCol)
	{
...
///echo $aryAfterResult[strtolower($aryDataset[$strValue]->name)].'<br /><br />';
			...
			// 事なるカラムをチェック
			if( $aryResult[strtolower($aryDataset[$strValue]->name)] != $aryAfterResult[strtolower($aryDataset[$strValue]->name)] )
			{
				$lngReturn++;	// 返却用（カラム数）
			}
