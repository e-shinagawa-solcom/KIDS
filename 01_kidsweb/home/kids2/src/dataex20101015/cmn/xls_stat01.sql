...
FROM
	...
	(
		...
		/* 受注明細・納品日に対する条件設定 */
		AND to_date( trd.dtmDeliveryDate,'YYYY/MM/DD' ) >= to_date( '_%dtmAppropriationDateFrom%_', 'YYYY/MM/DD' ) /* FROM */
		AND to_date( trd.dtmDeliveryDate,'YYYY/MM/DD' ) <= to_date( '_%dtmAppropriationDateTo%_', 'YYYY/MM/DD' ) /* TO */
	) mst1
