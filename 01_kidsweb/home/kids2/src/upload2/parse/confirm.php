...
//var_dump( get_object_vars($exc) );	// property
	...
	// 5桁に拡張してマスタ検索を実施する
	if (strlen($aryProductHeader["strproductcode"]) == 4)
	{
		$aryProductHeader["strproductcode"] = '0'.$aryProductHeader["strproductcode"];
	}
...
fncDebug( 'upload_parse_confirm_DiffProduct.txt', $aryDiffProduct, __FILE__, __LINE__);
	...
	{
		$msg = fncOutputError( 1601, DEF_WARNING, "", TRUE, "", $objDB);
	}
...
//		$dtmInsertDate	= fncGetMasterValue( "m_estimate", "strproductcode", "to_char( dtminsertdate, 'YYYY/mm/dd' ) as dtminsertdate",  $aryProductHeader["strproductcode"].":str", '', $objDB );
	...
	// ExcelワークシートHTML取得
	$strExcel	= getWorkSheet2HTML( $aryWSData, $strWSName, $ws_num, "confirm", $objExc );
...
fncDebug( 'upload_parse_confirm_Master.txt', $aryMaster, __FILE__, __LINE__);


	$strDiff	.= '<div id="confirm_header">';
	// 製品マスタとの差異が存在する場合
...
fncDebug( 'upload_parse_confirm_diff.txt', $strDiff, __FILE__, __LINE__);



	// テンプレート読み込み
	$objTemplate->getTemplate( "upload2/parse/confirm.tmpl" );
...
fncDebug( 'upload_parse_confirm.txt', $objTemplate->strTemplate, __FILE__, __LINE__);
	...
	// デストラクト
	$objMap->destruct();
	unset( $objExc, $objMap, $objDB, $objAuth, $objTemplate, $aryData, $strWorkSheet, $strExcel, $strForm );
