...
function fncGetSalesHeadNoToInfoSQL($lngSalesNo)
{
    ...
    $aryQuery[] = "  ) p ";
    $aryQuery[] = "    ON tsd.strProductCode = p.strProductCode ";
    $aryQuery[] = "    and tsd.strrevisecode = p.strrevisecode ";
...
function fncSetSalesDetailTabelData($aryDetailResult, $aryHeadResult)
{
    ...
    for ($i = 0; $i < count($aryColumnNames); $i++) {
        ...
        // 税額
			...
			var_dump($aryDetailResult["curtaxprice"]);
			var_dump(convertPrice($aryHeadResult["lngmonetaryunitcode"], $aryHeadResult["strmonetaryunitsign"], 0, "taxprice"));
