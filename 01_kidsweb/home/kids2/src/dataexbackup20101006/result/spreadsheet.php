			...
			for( $j = 0; $j < count($aryDataset); $j++ )
			{
				...
				switch( (int)$aryData["lngExportData"] )
				{
					...
					case 1:
						...
						if( $j == 17 || $j == 18 || $j == 19  )
						{
							$arySubTotalBuff[1][$j] += $varCellData;	// 顧客コードまとめ
							$arySubTotalBuff[2][$j] += $varCellData;	// 部門コードまとめ
							$arySubTotalBuff[3][$j] += $varCellData;	// 総計
						}
...
//					$arySubTitle = fncSpreadSheetCellData_StockSubTitle();
					...
					if( $lngRet > 0 )
					{
						...
						// 小計行の追加
						$lngAddLine = fncSpreadSheetCellData_SubTotalAdd($arySubTitle, $lngRet, $i, $lngHeadLineCnt, $lngSubtotal, $workbook, $worksheet, $aryFormat, $aryData, $aryDataset, $arySubTotalBuff, 16, 19 );
					}
					break;
